# Changes:
# disabled game_over prompt
# disabled runtime timing
# disabled database
# disabled detail
# added absorption count, change at line 30 and 158
# added action at line 160, 216
# delete parameter names (line 24, 31, 96)
# change the initial value of result from None to {} (line 47)
...
class World():
    ...
    def absorb(self, collision):
        ...
        for ele in collision:
            ...
            if biggest==0:
                self.player0absorbed+=1

    def update(self,frame_delta=Consts['FRAME_DELTA']):
        """Create new frames.
        Args:
        ...
        if self.timer[0] > 0:
            try:
                ...
                theta0=np.angle(z) if abs(z)>=1 else None
                # tf = pf()
                # self.timer[0] -= tf - ti
